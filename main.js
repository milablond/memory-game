class MemoryGame {
  ...
  bindEvents() {
    ...
    this.pairPopup.addEventListener('click', () =>
      this.hidePopup(this.pairPopup)
    );
  ...
  showDoubtTooltip(event) {
    const message =
      this.doubtMessages[Math.floor(Math.random() * this.doubtMessages.length)];
  ...
  checkMatch() {
    ...
    if (
      first.card.shape === second.card.shape &&
